...
def  load_cat():
    cat_model = pickle.load(open('bert_cat_class.sav', 'rb'))
    return cat_model
cat_model = load_cat()

@st.cache(allow_output_mutation=True)
def load_sent():
    sent_model = pickle.load(open('bert_sent_class.sav', 'rb'))
    return sent_model
...
def loadbert():
    ...
    return model_class, tokenizer_class, pretrained_weights, tokenizer, model

model_class, tokenizer_class, pretrained_weights, tokenizer, model = loadbert()
...
st.header('An aid towards streamlining positive, relevant Black news articles by classifying Black news category and sentiment from headlines')

st.subheader("Enter a headline title below and click 'classify' to view its predicted category (General,Sports, Business) and sentiment score!")

st.write('For example:')

titles = pd.Series(['Walmart Says It Will No Longer Lock Up African-American Beauty Products',"13 Investigates: 'No need for disparity' in African American arrest rates",'Timberwolves president Gersson Rosas hopeful for more front-office diversity in pro sports',
...
if st.button('Click button to generate sentiment score!'):
    ...
    ptokenized = titles.apply((lambda x: tokenizer.encode(x, add_special_tokens=True)))
    ...
    pred = sent_model.predict(features)
...
if st.button('classify'):
    ...
    #PREPROCESSING
        ...
        for i in range(len(pred)):
            st.write(titles,predictions[i],'|',sents[i])
